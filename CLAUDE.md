...
- **Seeing the GUI screenshots:** the cloud container can't download Actions artifacts. The GUI job therefore also prints each screenshot as `SHOT <name> <base64 chunk>` lines in its log (`DEBLOATIFY_PRINT_SCREENSHOTS`). Fetch the job log and join the chunks back into JPEGs. The CI screen is 1024x768, which makes it a good small-laptop check.
